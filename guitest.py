...
location = "potential awesome location"
classLine = ""
semester = ""
remindMins = 10
...
def parse(creds, filename, calid):
    ...
    while counter < end:
        ...
        print(nextDay)
        print(dayToInt(Day))
        createEvent(creds, localStart, localEnd, className, nextDay, dayToInt(Day), calid)
        lastDay = currDay
        counter += 1
    """
    #print("after run\n")
...
# creates a new calender in google calendar to not overwrite primary
def newCal(creds):
    service = build('calendar', 'v3', credentials=creds)
    calendar = {
            'summary': 'XE Registered',
...
def createEvent(creds, startDate, endDate, className, nextDay, dayInc, calid):
    service = build('calendar', 'v3', credentials=creds)
    ...
    event_result = service.events().insert(calendarId=calid,
        body={
            ...
            "reminders": { 'useDefault': False, 'overrides': [ {'method': 'popup', 'minutes': remindMins}],},
            "end": {"dateTime": end, "timeZone": "America/Chicago"},
    }).execute()
    with open('ids.txt', 'a') as f:
        f.write(event_result['id'])
        f.write("\n")
        f.close()
...
def openFile():
    filepath = filedialog.askopenfilename(filetypes= (("html files","*.html"),("text files","*.txt*")))
    print(filepath)
    main(filepath)
    window.destroy()
...
def gui():
    ...
    canvas.create_text(300, 100, text="Once you have completed registration", fill="white", font=('Helvetica 15 bold'))
    canvas.create_text(300, 150, text="Press command save (mac) or alt save (windows)", fill="white", font=('Helvetica 15 bold'))
